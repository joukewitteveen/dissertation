...
\begin{figure}
  ...
  \centering
  \subcaptionbox{??tetrahedron\label{fig:tetrahedron}}[0.3\textwidth]{
  ...
  \quad
  \subcaptionbox{??cube\label{fig:cube}}[0.3\textwidth]{
    \begin{tikzpicture}[line join=round,x={(1cm, .1494cm)},y={(-.5774cm, .2588cm)},z={(0cm, 1.1154cm)}]
      \draw[every edge/.append style={densely dashed, opacity=.5}]
  ...
  \quad
  \subcaptionbox{??dodecahedron\label{fig:dodecahedron}}[0.3\textwidth]{
...
%also: an abstract cube has no defined mass
...
For sets $A, B$, let $A \symdiff B$ denote the \defkey{symmetric difference} $(A \setminus B) \cup (B \setminus A)$.
...
Another problem \citeauthor{garey1979computers} look at is the \cl{NP}"~complete \pr{Clique} problem, which we encountered before in Example~\ref{ex:clique}.\indexkey{Clique@\pr{Clique}}
...
In the \pr{VertexCover} problem, we are given a pair~$\pair{G}{k}$ and are asked whether $G$ has a vertex cover of at most $k$ vertices.
...
Similarly, we could consider a parameterized version of the \pr{Clique} problem, where we parameterize by the solution size.
...
\begin{codelisting}
\item
  \code{Use} the fixed-parameter tractable algorithm for \pr{VertexCover} to find a vertex cover, $C$, of~$G$ that contains at most $k$ vertices.
\item
...
However, we were once again unable to analyze the parameterized computational complexity of the unmodified classical \pr{Clique} problem.
...
For this reason, a fixed-parameter tractable algorithm for, say, \pr{Clique} does have sufficient time to compute the minimum vertex cover size.
...
\begin{definition}
  The \emph{width} of a tree decomposition is one less than the number of elements in the largest set among the vertices of the decomposition.
...
However, the problem of deciding whether the treewidth of an arbitrary graph meets a given bound is itself \cl{NP}"~complete~\parencite{arnborg1987complexity}.
...
It has also been recognized that there is little reason why a parameterization is restricted in that it may only map to a single number.
