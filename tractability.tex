...
\label{def:uniform_order}%
  A parameterization~$\eta$ is below a parameterization~$\zeta$ in the \emph{uniform} order \defkeyat{<@$\quasile$}{$\quasile$} if there is a computable function bounding $\gap_{\eta, \zeta}$ from above.
\end{definition}
...
\indexkey{omni-levelable!for FPT@for \cl{FPT}|)}
...
To make this precise, let $\mathfrak{F}$ be a formal system capable of expressing statements about computation.\indexkey{formal system}
