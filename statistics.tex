...
Bounding the Kolmogorov complexity of a string using a computable estimator~$f$ is possible via the inverse images of~$f$.
...
\begin{equation*}
  \eta'_{f'(x)} = f'^{-1}(f'(x)).
\end{equation*}
Let us say that an estimator~$f$ for a quasiparameterization~$\eta$ that, for all~$x$, satisfies $\eta_{f(x)} = f^{-1}(f(x))$ is a \defkeyat{estimator!slice-estimator}{slice"~estimator}.
