...
If the class \cl{\itshape C} in Definition~\ref{def:bi-immune} is infinite, an immune set~$A$ satisfies infinitely many requirements.
...
A truth-table reduction that always makes precisely one query and outputs the result of that query is known as a \defkeyat{reduction!many--one}{many--one reduction}.
...
\begin{lemma}
  ...
  \end{equation*}
\end{lemma}
In this case, the variable~$y$ is known as the \defkey{advice} for instances of length~$n$.
...
The \emph{Berman--Hartmanis conjecture} \parencite{berman1977isomorphisms} posits that all sets that are complete for \cl{NP} are isomorphic to each other.
...
The class of sets that have a decision procedure of which the running time is bounded from above by~$f$ is denoted by~\cltime{$f(n)$}.
Hence, we have
\begin{equation*}
