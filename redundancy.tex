...
\label{ex:cylinder-kernel}%
  ...
  We might as well replace~$z$ with the constant string~\bits{0} of length one.
...
\label{ex:vertexcover}%
  ...
  \end{equation*}
  A polynomial many--one kernelization for \pr{VertexCover} with respect to the parameterization that bounds the size of the solution,
  \begin{equation*}
  ...
  \begin{codelisting}
  \item
    \code{If} $l > \asNat(k)$, \code{return} \bits{?}.
    We do not have $(G, l) \in \eta_k$, so we do not need to decide wether $(G, l)$ is a member of \pr{VertexCover} or not.
  \item
    \code{Remove} from~$G$ all isolated vertices.
    There is no reason to include any of these vertices in a vertex cover of~$G$.
  \item
    \code{Set} $j$ to the number of vertices of~$G$ that are connected to more than~$l$ others, and \code{remove} from~$G$ all these vertices.
  ...
  \item
    \code{If} more than $(l - j)(l + 1)$ vertices remain in~$G$, \code{return} \bits{0}.
  ...
  \item
    \code{Query} the oracle for $(G, l - j)$ and \code{return} the result.
  \end{codelisting}
  ...
  Also, note that this procedure converges to \pr{VertexCover} on~$\eta$.
  ...
  Therefore, the procedure is indeed a polynomial many--one kernelization for \pr{VertexCover}.
\end{example}

The queries made by a polynomial Turing kernelization are generally considered to be reasonably short~\parencite{flum2006parameterized}.
...
\begin{proof}
  ...
  Any set for which there is a kernelization of a size that is bounded from above by a constant is in~\cl{P}.
  ...
  Our set~$A$ is in \cl{FPT} with the parameterization
  \begin{equation*}
...
If this were not the case, then $A$ would not be bi"~immune for~\cl{E}.
...
\begin{proof}
  ...
  Note that these inputs satisfy $\bits{1}x \in \eta_{\asStr(n)}$, so if~$\phi_i$ is a kernelization with respect to~$\eta$, then it will not output~\bits{?}.
...
\slogan{With kernelizations, the queries that \emph{could} have been made matter too.}
...
This single set must withstand a form of autoreducibility and be sparse\indexkey{sparse set}.
...
\begin{proof}
  ...
  \item
    ...
    \begin{codelisting}
    \item
      \code{In case} $\phi_i$ returns \bits{?} or runs out of time, we cannot diagonalize against~$\phi_i$ at~$x$, and \code{continue} with the next value of~$i$ that satisfies $i \le I_\code{max}[x]$ and $i \notin I_\code{done}$.
    \item
...
\begin{proof}
  ...
  \item
    \code{Set} $k$ to $\asStr(\log \length{x})$, so that we have $\bits{1}x \in \eta_k$.
  \item
    \code{Run} $\phi(\bits{1}x, k)$ and \code{in case} it queries a string~$q$, respond as follows:
    \begin{codelisting}
    ...
    \end{codelisting}
    \item
      \code{Return} the opposite of the value returned by~$\phi(\bits{1}x, k)$.
  \end{codelisting}
  ...
  This means that the \code{if}"~clause of step~\ref{code:hpositive:not_self} in the above approximation algorithm for~$W$ is only satisfied for finitely many inputs~$w$.
  Consequently, the approximation outputs~\bits{?} for only finitely many inputs and the domain of the approximation must be infinite.
...
\begin{proof}
  ...
  \item
    ...
    \end{codelisting}
  \item
    \code{If} $\phi(\bits{1}x, k)$ returned \bits{0}, we infer that $w$ is not a member of $W$ and \code{return} \bits{0}.
  \item
  ...
  Hence, $\phi(\bits{1}x, k)$ must return~\bits{0} for infinitely many values of~$x$, showing that the domain of the approximation outlined above is infinite.
\end{proof}
...
\begin{figure}[bp]
 ...
 edges={draw=none}, edge quotes={sloped, allow upside down}]{
      ...
      p1 ->["$\subset$"] {
        ...
        "$\cl{PKER}_\textnormal{\rlap{$1$ query}\hphantom{psize}}$" ->["$\subset$"]
          2/"$\cl{PKER}_\textnormal{\rlap{$2$ queries}\hphantom{psize}}$" ->["$\subset$"]
          3/"$\cl{PKER}_\textnormal{\rlap{$3$ queries}\hphantom{psize}}$" ->["$\subset$"]
          "$\vdots$" ->["$\subset$"]
          psize/"$\cl{PKER}_\textnormal{psize}$" ->["$\subset$"]
          tt/"$\cl{PKER}_\textnormal{\rlap{truth-table}\hphantom{psize}}$" ->["$\subset$"]
          T/"$\cl{PKER}_\textnormal{\rlap{Turing}\hphantom{psize}}$" ->["$\subset$"]
          FPT/"$\cl{FPT}$"
      };
      p2 ->["$\subset$"] 2;
      p3 ->["$\subset$"] 3;
...
The name \enquote{psize kernelization} was coined by \textcite{witteveen2019hierarchy}, but some interesting properties of these kernelizations were already studied by \textcite[Chapter~5]{weller2013aspects}.
...
\label{thm:ppoly-tt}%
  ...
  \item\label{enum:advice:ppoly}
    $X$ is decidable and in \cladv{P}{poly}.
...
\begin{proof}
  ...
  \begin{codelisting}
  \item
    \code{If} $n$ is not a square, \code{set} $\chi(n)$ to
    \begin{equation*}
  ...
  \item
    ...
    \begin{codelisting}
    \item
      We gather the decisions of~$\phi_{\sqrt{n}}$ on strings of length~$n$ for all advice strings of a limited length and \code{set}
      \begin{equation*}
...
Because of this, Theorem~\ref{thm:ppoly-tt} and Theorem~\ref{thm:ppoly-one} show that the notions of polynomial advice represented by \cladv{P}{poly} and $\cl{PKER}_\textnormal{psize}$ are incomparable.
...
Because of this bound, having a weak \pr{or}"~distillation can be seen as a generalization of being \pdash{}selective, Definition~\ref{def:p-selective}.

Originally, distillations where considered where the target set~$B$ was equal to~$A$, hence the \enquote*{weak} in this more general definition.
...
When the framework behind Theorem~\ref{thm:distillation} was first published~\parencite{bodlaender2009problems}, it was immediately linked to the fact that \cl{NP}"~hard sets are unlikely to have weak distillations.
...
The history of the result by \citeauthor{fortnow2011infeasibility} can be traced back to results on \pdash{}selective sets by \textcite{selman1979p-selective,ko1983self}.\indexkey{p-selective@\pdash{}selective}
We remark that in a similar fashion, \textcite{hemaspaandra1995nondeterministically} had shown that sets with \emph{nondeterministic} selector functions are in~\cladv{($\cl{NP} \cap \cl{coNP}$)}{poly}.

A result like that of \citeauthor{fortnow2011infeasibility} for weak \pr{and}"~distillations was initially left as a conjecture~\parencite{bodlaender2009problems}.
This conjecture was proven by \textcite{drucker2015new}, who showed that \cl{NP}"~hard sets also do not admit weak \pr{and}"~distillations unless we have $\cl{NP} \subseteq \cladv{coNP}{poly}$.
...
\begin{example}
  ...
  \end{align*}
  In Example~\ref{ex:clique_vc}, we have seen that \pr{Clique} is in \cl{FPT} with the vertex cover parameterization.
  ...
  It is only there to align $\eta$ with \pr{Clique}.

  As shown by \textcite[Section~17.3.2]{fomin2019kernelization}, there is an \pr{or}"~cross composition with respect to the vertex cover parameterization of~\pr{Clique} into itself.
  Thus, by corollary~\ref{cor:lower_bound}, there is no psize kernelization for \pr{Clique} with respect to the vertex cover parameterization.
  This can be contrasted with Example~\ref{ex:vertexcover}, which shows that \pr{VertexCover} has a polynomial many--one kernelization.
\end{example}

Using the technique of \citeauthor{bodlaender2009problems}, we are unable to rule out any \emph{adaptive} kernelizations.
...
The proof of \citeauthor{dell2014satisfiability} only applies to \pr{or}"~cross-composition and a similar result for \pr{and}"~cross-composition remains open.
