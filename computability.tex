...
\begin{corollary}
\label{cor:decidable}
  Let \cl{C} be a complexity class in a parameterized framework that revolves around properties.
  A set (together with some parameterization) is in \cl{C} if and only if it is decidable.
\end{corollary}
...
\begin{definition}
  ...
  A parameterized procedure $\phi$ is \defkeyat{parameterized procedure!$f$-alternating}{$f$-alternating} if for every instance $x$ there are at most $f(x)$ values of $i$ satisfying
  \begin{equation*}
    \phi(x, \asStr(i)) \neq \phi(x, \asStr(i + 1)).
  \end{equation*}
\end{definition}
...
An parameterized procedure is convergent if it is $f$-alternating and for every instance $x$ outputs \bits{?} for only finitely many parameter values.
...
Such sets form the $\Delta^{-1}_n$ levels of a hierarchy known today as the \defkey{difference hierarchy} \parencite{downey2010algorithmic}.
...
The undecidable halting set\index{\pr{Halt}},
\begin{equation*}
...
\begin{proof}
  ...
  If the simulation terminates, $\phi$ outputs \bits{1}, else, it outputs \bits{0}.
...
\begin{corollary}
  Let \cl{C} be a fixed-parameter complexity class in a parameterized framework that revolves around promises.
  A set (together with some parameterization) is in \cl{C} if and only if it is in the $\Delta^0_2$ level of the arithmetical hierarchy.
\end{corollary}
...
In other words, there is a parameterization such that the subset of its index set corresponding to a particular subparameterization is \defkey{bi-immune} \parencite{rogers1967theory,odifreddi1992classical}.
...
\begin{definition}
  A set $A$ is \defkeyat{p-selective@\slp-selective}{\slp-selective} if there is a function $f\colon \binary^+ \times \binary^+ \to \binary^+$ that is computable in polynomial time and satisfies, for all strings $x, y$,
  \begin{itemize}
