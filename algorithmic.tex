\section{as Algorithmic Complexity}
...
\todo[inline]{
  ...
  The previous section looked at collections of parameterizations that make a set fixed-parameter tractable.
  Orthogonal to that, this section looks at sets that are made fixed-parameter tractable by a particular collection of parameterizations.
}
...
\begin{proof}
  ...
  Returning \bits{?} on other inputs, this procedure is a $p$"~approximation and its length serves as an upper bound on the instance complexity of $x$.
...
This conjecture posits that the Kolmogorov complexity\indexkey{Kolmogorov complexity|(} is infinitely often within an additive constant of the instance complexity.
...
\todo[inline]{
  Put some text here.
  %Remember: we look an \symdiff in \cl{P} because we are really interested in sets that have the same distribution of computational complexity (relative to \cl{P}).
  %\parencite{goldsmith1996scalability}
  %We conjecture that any distribution of complexity is unique, not up to polytime-isomorphisms (Berman--Hartmanis), but up to polytime-postprocessing.
}
\begin{theorem}
...
We could say that a set in \cl{P} separates two sets if it contains a polytime-core for precisely one of the two.
...
\label{con:fptsymdiff}%
  ...
  \end{equation*}
\end{conjecture}
%TODO: Also add a slogan? The computational complexity behavior is reflected by the collection of parameterizations (that put a set in \cl{FPT}).
...
Conjecture~\ref{con:fptsymdiff} suggests that a notion of \emph{having the same complexity} is provided by the kernel of the function that maps a set $A$ to the filter $\calF_\cl{FPT}(A)$.
...
\begin{definition}
  A set of strings $X$ is \defkeyat{p-printable@\pdash{}printable}{\pdash{}printable} if there is a polynomial $p$ and a procedure $\phi$ that, on any input $\asStr(n)$, lists all elements of $X$ up to size $n$ within $p(n)$ steps.
\end{definition}
...
\end{theorem}
%TODO: Note that the set $X$ in the theorem above is not actually a set with small generalized Kolmogorov complexity as the time bound is not polynomial.
%      However, the time bound can be close to polynomial and the difference between $X$ and a similar set with small generalized Kolmogorov complexity is slim.
